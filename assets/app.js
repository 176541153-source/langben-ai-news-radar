...
const briefLinkEl = document.getElementById("briefLink");
...
function decisionLabel(decision) {
  ...
  return labels[decision] || decision || "观察";
}

function renderSourceQualityTable(status) {
  const rows = Array.isArray(status?.source_quality) ? status.source_quality : [];
  if (!rows.length) return null;

  const wrap = document.createElement("div");
  wrap.className = "quality-table";

  const head = document.createElement("div");
  head.className = "quality-row quality-head";
  ["来源", "AI/全量", "命中率", "建议"].forEach((text) => {
  ...
  rows.slice(0, 10).forEach((row) => {
    ...
    line.className = `quality-row decision-${row.decision || "watch"}`;

    const name = document.createElement("span");
    name.textContent = row.site_name || row.site_id || "来源";

    const count = document.createElement("span");
    count.textContent = `${fmtNumber(row.ai_24h || 0)}/${fmtNumber(row.raw_24h || 0)}`;

    const rate = document.createElement("span");
    rate.textContent = `${Math.round(Number(row.ai_hit_rate || 0) * 1000) / 10}%`;

    const decision = document.createElement("span");
    decision.className = "quality-decision";
    decision.textContent = decisionLabel(row.decision);
    decision.title = row.decision_reason || "";

    line.append(name, count, rate, decision);
...
function renderSourceHealth(errorMessage = "") {
  ...
  const qualitySummary = status.source_quality_summary || {};
  ...
  metricGrid.append(
    ...
    renderMetric("保留源", fmtNumber(qualitySummary.keep || 0), "ok"),
    renderMetric("降权/修复", `${fmtNumber(qualitySummary.downgrade || 0)}/${fmtNumber(qualitySummary.fix_fetch || 0)}`, (qualitySummary.downgrade || qualitySummary.fix_fetch) ? "warn" : "")
  );
  sourceHealthEl.appendChild(metricGrid);

  const qualityTable = renderSourceQualityTable(status);
  if (qualityTable) sourceHealthEl.appendChild(qualityTable);
...
async function init() {
  ...
  if (newsResult.status === "fulfilled") {
    ...
    if (briefLinkEl && payload.daily_brief_url) briefLinkEl.href = `./${payload.daily_brief_url}`;
  } else {
