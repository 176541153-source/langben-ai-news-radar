...
class AiRelevanceScoringTests(unittest.TestCase):
    ...
    def test_accepts_custom_ai_brand_keywords(self):
        rec = {
            "site_id": "trendradar",
            "site_name": "TrendRadar 中文热榜",
            "source": "微博",
            "title": "Nano Banana 图像模型能力曝光",
            "url": "https://example.com/nano-banana",
        }
        result = score_ai_relevance(rec)
        self.assertTrue(result["is_ai_related"])
        self.assertGreaterEqual(result["score"], 0.78)
        self.assertEqual(result["reason"], "matched_tracked_ai_keyword")
        self.assertIn("Google Gemini", result["topic_labels"])

    def test_accepts_custom_internet_company_keywords(self):
        rec = {
            "site_id": "trendradar",
            "site_name": "TrendRadar 中文热榜",
            "source": "百度",
        ...
        self.assertTrue(result["is_ai_related"])
        self.assertGreaterEqual(result["score"], 0.65)
        self.assertEqual(result["reason"], "matched_tracked_internet_keyword")
        self.assertEqual(result["label"], "industry_business")
        self.assertIn("华为", result["topic_labels"])

    def test_ai_regex_does_not_match_inside_english_words(self):
        rec = {
            "site_id": "buzzing",
            "site_name": "Buzzing",
            "source": "general",
            "title": "Said company unveils new railway plan",
            "url": "https://example.com/said-railway",
        }
        result = score_ai_relevance(rec)
        self.assertFalse(result["is_ai_related"])
    ...
    def test_adds_public_debug_fields(self):
        ...
        self.assertIn("topic_labels", out)
