...
- 输出维护者可读的 `daily-brief.zh.md`、`radar-brief.json`、`archive-index.json` 和源质量建议
- 新增 `radar.html` 伯乐雷达承接页，用故事合并、分类板块和新增信号承接完整 pipeline
- 新增 `config/upstream-sources.json` 四项目后台接入配置，自动汇入 AI HOT Skill、AI News Radar、TrendRadar 和 Horizon
- 中英双语标题和站点分组
...
如果要使用页面里的“立即更新”按钮，用本地更新服务替代静态服务：

```bash
node scripts/radar-update-server.mjs 8080
```

它会提供 `POST /api/update`，点击按钮时自动执行 `scripts/update_news.py` 并刷新页面数据。GitHub Pages 线上仍是静态站，不能直接执行服务器脚本。
...
- 默认每 30 分钟运行一次
- 页面里的“立即更新”按钮只在本地 `scripts/radar-update-server.mjs` 服务下生效
- 自动生成并提交 `data/*.json`
- 额外生成 `data/daily-brief.zh.md` 日报、`data/radar-brief.json` 承接页数据和 `data/archive-index.json` 归档索引
- 额外生成 `data/upstream-hub.json`，保留四个上游项目的后台接入数据
...
本项目已配置为 Cloudflare Pages 静态部署，目标域名为 `https://ainews.caodiu.com/`。

```bash
node scripts/build-cloudflare-pages.mjs
npx wrangler pages deploy dist --project-name langben-ai-news-radar --branch master
```

Cloudflare Pages 需要绑定自定义域名 `ainews.caodiu.com`，根路径会直接承接到 `radar.html`。`cloudflare/_worker.js` 提供远程 `/api/status` 和 `/api/update`：

- 自动更新仍由 GitHub Actions 每 30 分钟执行
- 页面上的“立即更新”在 Cloudflare 上会触发 GitHub `update-news.yml`
- Cloudflare Pages 环境变量需要配置 `GITHUB_TOKEN`、`GITHUB_OWNER`、`GITHUB_REPO`、`GITHUB_REF=main`
- 配置完成后，Cloudflare Worker 会优先从 GitHub 仓库读取最新 `data/*.json` 和日报 Markdown；GitHub Actions 提交数据后无需重新部署 Pages 即可刷新前台数据
- GitHub 仓库 Secrets 可选配置 `CLOUDFLARE_API_TOKEN`、`CLOUDFLARE_ACCOUNT_ID`，用于工作流完成后顺手部署静态资源
- GitHub 仓库 Variables 可选配置 `CLOUDFLARE_PAGES_PROJECT=langben-ai-news-radar`
